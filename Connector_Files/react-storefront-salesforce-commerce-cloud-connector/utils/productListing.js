...
import normalizeProduct from './normalizeProduct'; // TODO: make process env var
...
export default async function productListing(params, req, searchOptions) {
  ...
  const totalPages = Math.ceil(search.total / limit) + 1; // collect all page data
  ...
  return {
    ...
    sort,
    sortOptions: search.sortingOptions.map(({
      label,
      id
    }) => {
      return {
    ...
    facets: (search.refinements || []).filter(e => e.values).map(({
      label,
      attributeId,
      values
    }) => {
