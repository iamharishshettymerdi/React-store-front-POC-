export default function normalizeProduct(data, color, size) {
  ...
  function getImages(viewType, variation, value) {
    ...
    groups.filter(group => {
      ...
      if (variation) {
        return group.variationAttributes && group.variationAttributes[0].id === variation && group.variationAttributes[0].values[0].value === value;
      }

      return true;
    }).reduce((memo, e) => [...memo, ...e.images], []).forEach(({
      link,
      alt
    }) => {
      images[link] = alt;
    });
    return Object.keys(images).map(src => ({
      src,
      alt: images[src]
    }));
  }
  ...
  const specs = Object.keys(data).filter(key => key.indexOf('c_') === 0).map(key => {
...
//# sourceMappingURL=normalizeProduct.js.map